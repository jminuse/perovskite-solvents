...
write_restart '''+run_name+'''.restart''')
	...
	os.system('/fs/home/bas348/lammps/src/lmp_serial -in '+run_name+'.in -log '+run_name+'.log')
	...
	molecules_in_cluster = [m[1] for m in molecules_in_cluster[ : (11 if solute else 10) ]]
...
#fix motion mobile npt temp 300.0 300.0 100.0 aniso 1.0 1.0 1000.0
fix motion mobile nvt
-+
run 1000
	qzXCDFVGTHYASDFGHJKL;'
	/.MNBGVFCDXSAZxcvb
minimize 1.0e-4 1.0e-6 1000 10000
...
write_restart '''+system.name+'''.restart''')
	...
	os.system('/fs/home/bas348/lammps/src/lmp_serial -in '+system.name+'.in -log '+system.name+'.log')
	...
	dielectrics = dict( zip(['acetone','ACN','DMF','dmso', 'gbl', 'methacrolein', 'nmp'], [20.7, 37.5, 36.7, 46.7, 40.24, 10.9 ,32.2]) )
	extra_section='%%cosmo  SMD true  epsilon %f  end' % dielectrics[solvent_name]
	orca.job(run_name, '! OPT B97-D3 SV GCP(DFT/SV) ECP{def2-TZVP} COSMO Grid3 FinalGrid5 SlowConv LooseOpt', system.atoms, queue=None, extra_section=extra_section, charge_and_multiplicity='2 1')
